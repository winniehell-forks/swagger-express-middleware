...
function sendObject(req, res, next, obj) {
  setContentType(req, res, 'json', function () {
...
function sendText(req, res, next, data) {
  setContentType(req, res, 'text', function () {
...
function sendFile(req, res, next, file) {
  if (file instanceof Buffer) {
    setContentType(req, res, 'application/octet-stream', function () {
 ...
 * @param   {Request}   req
 * @param   {Response}  res
 * @param   {string}    defaultType  - fallback MIME type
 * @param   {function}  callback
 */
function setContentType (req, res, defaultType, callback) {
  // Get the MIME types that this operation produces
  var supportedTypes = req.swagger.operation.produces || req.swagger.api.produces || [];

  if (supportedTypes.length === 0) {
    // No MIME types were specified, so just use the first one
    util.debug('No "produces" MIME types are specified in the Swagger API, so defaulting to %s', defaultType);
    supportedTypes = [defaultType];
  }

  var responseType = req.accepts(supportedTypes);
  if (!responseType) {
